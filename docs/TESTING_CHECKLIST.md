...
| TC-SEC-004 | Controlled Input Lengths | Any Role | Textarea & Text Inputs | Enforce maximum input length limits on form text fields | 1. Attempt typing excessive text into Q&A (500), Document Upload (120/1000), NCR fields (120/2000), or Feedback comment (1000). | Controlled input | Enforces controlled maximum input lengths (Q&A: 500, Title: 120, DocNum: 40, Rev: 20, Dept: 80, Site: 80, Tags: 250, Excerpt: 1000, NCR Part: 120, NCR Supplier: 160, NCR Issue: 2000, Containment: 2000, Feedback: 1000). Excessive input is capped via maxLength and rejected with validation error text. | Verified maxLength HTML attributes and validation checks prevent excessive input length across all text inputs and textareas. | Pass | SEC-REC-001 Implemented (AI-assisted Playwright browser test) |
...
| TC-NET-002 | Storage Fallback | Any Role | Disabled LocalStorage | Verify app behavior when browser storage is restricted or disabled | 1. Disable `localStorage` in browser settings or use private mode with storage blocked.<br>2. Perform app actions. | Storage blocked | App functions correctly in memory without throwing unhandled JS exceptions and displays a dismissible storage persistence warning banner ("Browser storage is restricted or full..."). | Blocked localStorage setItem handler. App displayed dismissible storage warning banner and continued functioning cleanly in memory. | Pass | SEC-REC-002 Implemented (AI-assisted Playwright browser test) |
